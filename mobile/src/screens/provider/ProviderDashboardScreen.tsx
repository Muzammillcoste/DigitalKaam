...
export function ProviderDashboardScreen({ navigation }: any) {
  ...
  const getFilteredBookings = () =>
    bookings.filter((item) => {
      if (activeTab === 'pending') return item.status === 'confirmed';
      if (activeTab === 'active')
  ...
  const calculateStats = () => {
    ...
    return { completedCount: completed.length, earnings };
  };

  const getStatusDetails = (status: BookingStatus) => {
    const statusMap: Record<BookingStatus, { label: string; color: string; icon: string }> = {
      confirmed: { label: 'Assigned', color: c.primary, icon: 'calendar-outline' },
      en_route: { label: 'On The Way', color: c.warning, icon: 'bicycle-outline' },
      arrived: { label: 'Arrived', color: c.info, icon: 'pin-outline' },
      in_progress: { label: 'In Progress', color: c.statusInProgress, icon: 'construct-outline' },
      completed: { label: 'Completed', color: c.success, icon: 'checkmark-circle-outline' },
      cancelled: { label: 'Cancelled', color: c.error, icon: 'close-circle-outline' },
    };
    return statusMap[status] || { label: status, color: c.textSecondary, icon: 'help-circle-outline' };
  };
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              <Ionicons name="briefcase-outline" size={48} color={c.textDisabled} />
...
const makeStyles = (c: ColorPalette) =>
  StyleSheet.create({
    ...
    statVal: { ...Typography.h3, color: c.primary },
    statLbl: { ...Typography.caption, color: c.textSecondary },
    ...
    tabButtonActive: { borderBottomColor: c.primary },
    ...
    jobUser: { gap: 2 },
    userName: { ...Typography.bodyLarge, fontWeight: '700', color: c.text },
    ...
    jobDetails: { gap: Spacing.xs, borderTopWidth: 1, borderTopColor: c.divider, paddingTop: Spacing.sm },
    detailRow: { flexDirection: 'row', alignItems: 'center', gap: Spacing.xs },
    detailText: { ...Typography.body, color: c.textSecondary },
    descText: { ...Typography.body, color: c.textSecondary, fontStyle: 'italic', marginTop: 2 },
    ...
    priceLbl: { ...Typography.caption, color: c.textSecondary, fontWeight: '600' },
    priceVal: { ...Typography.bodyLarge, fontWeight: '800', color: c.text },
    ...
    emptySubtitle: { ...Typography.body, color: c.textSecondary, textAlign: 'center', lineHeight: 20 },
  });
